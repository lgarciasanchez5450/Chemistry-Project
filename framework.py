...
class Button:
  ...
  def recalcBounds(self):
    ...
    self.textpos = (self.x+self.textx + self.offSetPos[0],self.y + self.texty + self.offSetPos[1] )
...
class GridComponent:
  ...
  def accepts():
    return 'mpos','mb1down','mb3down','KDQueue','mb1up'
  def __init__(self,pos:tuple[int,int],cells:tuple[int,int],cellSize:tuple[int,int]) -> None:
    self.x = pos[0]
    self.y = pos[1]
  ...
  @offSetPos.setter
  def offSetPos(self,val:tuple[int,int]):
    for button in self.buttons:
  ...
  def addButton(self,pos:tuple[int,int],button:Button):
    x,y = pos
    index = self._xyToIndex(x,y)
    if self.buttons[index] is not None: raise RuntimeError("Cell Already Full")
    button.offSetPos = (self._offSetPos[0]+self.x,self._offSetPos[1]+self.y)
    button.x = x * (self.cellSize[0] + self.spacing)
    button.y = y * (self.cellSize[1] + self.spacing)
    button.xlen = self.cellSize[0]
...
class Window_Space:
    ...
    @property
    def top(self) -> 'Top_Border': return self._borders['top']
    @property
    def left(self) -> 'Left_Border': return self._borders['left']
    @property
    def right(self) -> 'Right_Border': return self._borders['right']
    @property 
    def bottom(self) -> 'Bottom_Border': return self._borders['bottom']
    @property
